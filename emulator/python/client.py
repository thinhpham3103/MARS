...
s.settimeout(1)
srvr = ( b'localhost', 21345 )

# send the DEVID
# s.sendto(mid, srvr)
# send the MarsID and BankSize
s.sendto(cbor.dumps((mid, mars.bsize)), srvr)

# read the NONCE
blob, addr = s.recvfrom(1024)
nonce, pcrsel = cbor.loads(blob)
print('Server:', addr)
...
# aggregate events matching PCRs in pcrsel from elog into CEL
def genCEL(pcrsel):
    cel = [] # array of CELR (CEL Records)
    for i in range(mars.bsize):
        if (1<<i) & pcrsel:
